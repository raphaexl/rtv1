...
/*   Updated: 2019/05/20 17:09:02 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_box_compute(t_object *p, t_intersect *in)
{
	...
	in->n = ft_normal_box(p, ft_vector_sub(in->p, p->pos));
...
int			ft_box_intersect(t_object *b, t_ray *r, float *min)
{
	...
	bounds[0] = b->pos;
	bounds[1] = b->normal;
	r->dir = (t_vector){1.0 / r->dir.x, 1.0 / r->dir.y, 1.0 / r->dir.z};
	sign[0] = r->dir.x < 0.00001;
	sign[1] = r->dir.y < 0.00001;
	sign[2] = r->dir.z < 0.00001;
	t[0].x = (bounds[sign[0]].x - r->start.x) * r->dir.x;
	t[1].x = (bounds[1 - sign[0]].x - r->start.x) * r->dir.x;
	t[0].y = (bounds[sign[1]].y - r->start.y) * r->dir.y;
	t[1].y = (bounds[1 - sign[1]].y - r->start.y) * r->dir.y;
	if ((t[0].x > t[1].y) || (t[0].y > t[1].x))
		return (0);
	t[0].x = (t[0].y > t[0].x) ? t[0].y : t[0].x;
	t[1].x = (t[1].y < t[1].x) ? t[1].y : t[1].x;
	t[0].z = (bounds[sign[2]].z - r->start.z) * r->dir.z;
	t[1].z = (bounds[1 - sign[2]].z - r->start.z) * r->dir.z;
