...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/05 14:28:00 by ebatchas          #+#    #+#             */
/*   Updated: 2019/05/10 12:30:01 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++x < W_W)
	{
		...
		while (++y < W_H)
		{
			...
			while (--s->a >= 0)
			{
				...
				s->c = ft_color_sum(s->c, ft_edit_trace(s, &inter, 0));
...
static	int		ft_rend(void *ptr)
{
	...
	while (--y >= g->start)
	{
		...
		while (--x >= 0)
		{
			...
			g->s.c = (t_color){0.0, 0.0, 0.0};
			while (--g->s.a >= 0)
			{
				inter.ray = ft_camera_ray(&g->s.cam, x + (float)g->s.a /
						g->s.nb_samples, y + (float)g->s.a / g->s.nb_samples);
				g->s.c = ft_color_sum(g->s.c, ft_edit_trace(&g->s, &inter, 0));
				//g->s.c = ft_color_sum(g->s.c, g->s.ft_rtv1(&g->s, &inter, 0));
			}
