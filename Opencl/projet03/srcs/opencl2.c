...
/*   Updated: 2019/05/10 19:54:34 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_opencl_mem(t_opencl *opencl)
{
	...
	opencl->w = opencl->scene.scr_width;
	opencl->h = opencl->scene.scr_height;
	opencl->scene_mem_obj = clCreateBuffer(opencl->context, CL_MEM_READ_ONLY, 1 * sizeof(t_scene), NULL, &ret);
	opencl->pixels_mem_obj = clCreateBuffer(opencl->context, CL_MEM_WRITE_ONLY, opencl->w * opencl->h * sizeof(uint), NULL, &ret);
	ret = clEnqueueWriteBuffer(opencl->command_queue, opencl->scene_mem_obj, CL_TRUE, 0, 1 * sizeof(t_scene), &opencl->scene, 0, NULL, NULL);
	if (ret != CL_SUCCESS)
		perror("debile");
}
...
void	ft_opencl_kernel(t_opencl *opencl)
{
	...
	ret = clSetKernelArg(opencl->kernel, 2, sizeof(cl_uint), (void *)&opencl->w);
	ret = clSetKernelArg(opencl->kernel, 3, sizeof(cl_uint), (void *)&opencl->h);
}
