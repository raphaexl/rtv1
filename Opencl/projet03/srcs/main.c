...
/*   Updated: 2019/05/10 19:52:15 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(int argc, char *argv[])
{
	...
	opencl.scene.cam = ft_camera_perspective(ft_vector(0.0f, 1.0f, -5.0f),
			...
			(cl_float)opencl.w / (cl_float)opencl.h);
	if (ft_sdl_init(&ptr, opencl.w, opencl.h, "42"))
		goto Quit;
	if (!(pixels = (Uint32 *)malloc(sizeof(Uint32) * opencl.w * opencl.h)))
		perror("Error Malloc pixels");
	memset(pixels, 0xFFFF00, sizeof(Uint32) * opencl.w * opencl.h);
	...
	{
		ft_update_event(&in);
		ft_update_renderer(ptr, pixels, opencl.w);
	//	ft_draw(&s, pixels);
