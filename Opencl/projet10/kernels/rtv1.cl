...
float		ft_pow(float nb, int puiss);
...
int				ft_cone_intersect(t_object *c, t_ray *r, float *t)
{
	...
	anglecos = ft_pow(cos(c->angle), 2);
	anglesin = ft_pow(sin(c->angle), 2);
...
t_color	ft_light(global t_scene *s, t_intersect *in, t_material m, t_color c)
{
	...
	while (p)
	{
		...
		if ((in->t = sqrt(ft_vector_dot(dist, dist))) && in->t <= 0.001)
		{
			p = p->next;
			continue;
		}
...
__kernel void		ft_render(global t_scene *s, __global float3 *pixels, unsigned int w, unsigned int h)
{
	...
	pixels[i] = (float3)(fx, fy, 0);

	//pixels[x + (W_H - 1 - y) * W_W] = (float3)(fx, fy, 0);
	...
	*/const	int i = get_global_id(0);
	...
	while (--s->a >= 0)
	{
		...
		s->c = ft_color_sum(s->c, s->ft_rtv1(s, &inter, 0));
	}
	pixels[x + (W_H - 1 - y) * W_W] = ft_clamp_gama(s->c.red, s->c.green, s->c.blue, 1.0 / s->nb_samples);
}
