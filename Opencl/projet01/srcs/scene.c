...
/*   Updated: 2019/03/19 14:41:47 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_scene_init_default(t_scene *s)
{
	...
	s->nb_lights = 3;
	s->nb_planes = 1;
	if ((s->planes = (t_plane *)malloc(sizeof(t_plane) * s->nb_planes)))
		ft_memcpy(s->planes, planes, s->nb_planes * sizeof(t_plane));
	else
		ft_error();
	if ((s->spheres = (t_sphere *)malloc(sizeof(t_sphere) * s->nb_spheres)))
		ft_memcpy(s->spheres, spheres, s->nb_spheres * sizeof(t_sphere));
	else
		ft_error();
	if ((s->lights = (t_light *)malloc(sizeof(t_light) * s->nb_lights)))
		ft_memcpy(s->lights, lights, s->nb_lights * sizeof(t_light));
	else
		ft_error();
	if ((s->materials = (t_material *)malloc(sizeof(t_material) * s->nb_materials)))
...
void	ft_scene_clear(t_scene *s, Uint32 **pixels)
{
	free(s->spheres);
	s->spheres = NULL;
	free(s->materials), s->materials = NULL;
