...
/*   Updated: 2019/05/10 14:22:11 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_opencl_mem(t_opencl *opencl, t_scene *s)
{
	...
	opencl->scene_mem_obj = clCreateBuffer(opencl->context, CL_MEM_WRITE_ONLY, sizeof(t_scene), NULL, &ret);
	opencl->pixels_mem_obj = clCreateBuffer(opencl->context, CL_MEM_READ_WRITE, W_W * W_H * sizeof(uint), NULL, &ret);
