...
/*   Updated: 2019/05/10 18:59:24 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color		ft_trace_v1(t_scene	*s, t_intersect *in)
{
	...
	while (coef > 0.0f && level < 15)
	{
		...
		if (id == -1 || s->obj == NONE)
			break;
		cl_float    temp = ft_vector_dot(in->n, in->n);
		if (temp == 0)
			break;
		temp = 1.0 / sqrt(temp);
		...
		while (++j < s->nb_lights)
		{
			...
			cl_float t = sqrt(ft_vector_dot(dist, dist));
			if (t <= 0.0f)
				continue;
			...
			if (!in_shadow)
			{
				cl_float lambert = ft_vector_dot(in->ray_light.dir, in->n) * coef;
		...
		in->ray.start = in->p;
		cl_float reflected = 2.0f * ft_vector_dot(in->ray.dir, in->n);
